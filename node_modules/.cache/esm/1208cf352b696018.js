 ...
 */
    ...
    checkLose() {
        ...
        board.forEach((val, ind) => {
            if (ind == 0) {
                if(val == board[ind+1] || val == board[i+this.width]) {
                    duplicates = true;
                }
            } else if (ind < this.width && ind % this.width-1 != 0) {
                if(val == board[ind+1] || val == board[i+this.width] || val == board[i-1]) {
                    duplicates = true;
                }     
            } else if (ind == this.width-1) {
                if(val == board[ind-1] || val == board[i+this.width]) {
                    duplicates = true;
                }  
            } else if (ind % this.width == 0 && ind >= this.width && ind <= this.width*(this.width-2)) {
                if(val == board[ind+1] || val == board[i+this.width] || val == board[i-this.width]) {
                    duplicates = true;
                } 
            } else if (ind >= this.width+1 && ind <= this.width*(this.width-1)-2) {
                if(val == board[ind+1] || val == board[i+this.width] || val == board[i-this.width] || val == board[ind-1]) {
                    duplicates = true;
                } 
            } else if (ind == this.width*(this.width-1)) {
                if(val == board[ind+1] || val == board[i-this.width]) {
                    duplicates = true;
                } 
            } else {
                if(val == board[ind-11] || val == board[i-this.width]) {
                    duplicates = true;
                } 
            }
